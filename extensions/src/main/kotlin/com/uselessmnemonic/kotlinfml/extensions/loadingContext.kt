...
val <M: IKotlinMod> M.loadingContext get(): KotlinModLoadingContext<M> {
    val contextExtension = KotlinModLoadingContext.getContext().orElseThrow {
        throw IllegalAccessException("caller is not in a Kotlin mod")
    }
    if (contextExtension.mod !== this) {
        throw IllegalAccessException("caller in mod ${contextExtension::javaClass.name} cannot access context for ${this::javaClass.name}")
    }
